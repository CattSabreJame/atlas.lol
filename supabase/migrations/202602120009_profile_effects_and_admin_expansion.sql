...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'profiles_profile_effect_check'
  ) then
    alter table public.profiles
      add constraint profiles_profile_effect_check
      check (profile_effect in ('none', 'glow', 'grain', 'scanlines', 'lifted'));
  end if;
end $$;

alter table public.admin_action_notices
  drop constraint if exists admin_action_notices_action_type_check;

alter table public.admin_action_notices
  add constraint admin_action_notices_action_type_check
  check (
    action_type in (
      'ban',
      'unban',
...
begin
  ...
  if normalized_action = 'ban' then
    ...
    where p.id = target_id;
  elsif normalized_action = 'reset_visuals' then
    update public.profiles p
    set
      avatar_url = null,
      profile_effect = 'none',
      ...
      background_value = null
    where p.id = target_id;
  elsif normalized_action = 'clear_bio' then
    update public.profiles p
    set
      bio = null,
      rich_text = ''
    where p.id = target_id;
  elsif normalized_action = 'wipe_links' then
    delete from public.links l where l.user_id = target_id;
  elsif normalized_action = 'purge_comments' then
    delete from public.comments c where c.user_id = target_id;
